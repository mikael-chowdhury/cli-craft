import { CheckBoxChoice, CheckBoxOptions } from "types";
import chalk from "chalk";
import readline from "readline";
import { PromptBase } from "../input/PromptBase";

const renderChoice = (choice: string, value: boolean, newline = true) => {
  process.stdout.write(
    ` ${chalk.greenBright(value == true ? "◉" : "○")} ` +
      choice +
      (newline ? "\n" : "")
  );
...
const CheckBox = (options: CheckBoxOptions): Promise<CheckBoxChoice[]> => {
  return new Promise((res, rej) => {
    const choices: CheckBoxChoice[] = options.choices.map((choice) => {
      if ((choice as CheckBoxChoice).name) return choice;
    ...
    options.choices.forEach((option, index) => {
      const choice = (option as CheckBoxChoice).name || (option as string);
      renderChoice(
        index == 0 ? chalk.cyan(choice) : choice,
        (option as CheckBoxChoice).value
          ? (option as CheckBoxChoice).value
          : false
      );
      newlines++;
    ...
    process.stdout.write(
      `\n\n${chalk.dim(
        "   use arrow keys to select option, enter to finalise\n"
      )}`
    );
    newlines += 3;
    ...
    const callback = (data: Buffer, key: any) => {
      if (key && key.name == "c" && key.ctrl) {
        ...
        if (currentY != choices.length) {
          renderChoice(
            chalk.cyan(choices[currentY].name),
            choices[currentY].value,
            false
          );
        } else {
